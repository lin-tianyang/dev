...
rm -f $pwfile/*.gz
#wst
aws s3 sync  s3://gt-cdn-log/$project-client/   $pwfile/    --exclude "*" --include E1I27K3SJOVW23.$cdnyear-$cdnmount-$cdnday*
#ksr
#aws s3 sync  s3://gt-cdn-log/$2/ $pwfile/    --exclude "*" --include "E1QF2CRVIRNQC0.$cdnyear-$cdnmount-$cdnday*"
cd $pwfile

#
sourcedate=`zcat *.gz | grep -Evw 'xxx1|x-edge-location|#Version' | awk '{print $4,$5}'`
#查看节点情况
node=`zcat *.gz | grep -Evw 'xxx1|x-edge-location|#Version' | awk '{print $3}' |  sort | uniq -c |awk '{ print $2","$1}' | sort -t , -k2nr`
#查看ip数量
ip=`echo "$sourcedate" | awk '{print $2}' | sort | uniq |wc -l`

#查看获取源的比例
miss=`zcat *.gz | grep -Evw 'xxx1|x-edge-location|#Version' | grep -iw Miss | awk '{print $5}' | sort | uniq |wc -l`


#根据ip统计文件大小
#filesize=`echo "$sourcedate" | awk  '{sum[$2]+=$1}END{for(c in sum){print c,sum[c]}}' |  sort -k2nr | awk '{print $1","($2/1024/1024)}' |  sort -t , -k2nr`     
#完全排除去重下载文件数量
num_uniq=`zcat *.gz | grep -Evw 'xxx1|x-edge-location|#Version' |awk '{print $5,$8}' |  awk '!a[$1" "$2]++{print}' | awk '{print $1}' | sort | uniq -c |  awk '{ print $2","$1}' |sort -t , -k1nr`
#不去重下载文件数量
num=`echo "$sourcedate" | awk '{print $2}' | sort | uniq -c |   awk '{ print $2","$1}' |sort -t , -k1nr` 
#统计ip下载时间和文件大小
#time-taken=`zcat * | grep -Evw 'xxx1|x-edge-location|#Version' |awk '{print $19,$5}' |awk  '{sum[$2]+=$1}END{for(c in sum){print c,sum[c]}}' |sort -t , -k2nr`
#花费时间和文件大小
time_taken=`zcat *.gz | grep -Evw 'xxx1|x-edge-location|#Version' | awk '{print $4","$5","$8","$19}' | sort -t , -k2nr | awk -F ',' '{s[$2] += $1; a[$2] += $4; }END{ for(i in s){  print i","s[i]","a[i] } }' |  sort -t , -k1nr`


#错误源文件
error=`zcat *.gz | grep -Evw 'xxx1|x-edge-location|#Version' | grep -i error`
#错误次数
...
zcat *.gz | grep -Evw 'xxx1|x-edge-location' | sort -k5nr | grep -i error -C 3 | awk '{print $2","$3","$4/1024/1024","$5","$8","$9","$14","$19","$23","$29}'   >> /data/test/$project/$project-cdn/wsterror-$cdnyear-$cdnmount-$cdnday.csv
